...
export class SophosURLProtector {
  ...
  protectURL(originalURL, options = {}) {
    try {
      ...
      console.log('📤 Protected URL parameters:');
  ...
  // ... rest of the methods remain the same (encryptData, decryptData, generateSecurityToken, etc.)
  ...
  constructSophosURL(params) {
    try {
      ...
      console.log('📤 Parameters for URL construction:');
      ...
      console.log('   - urlId:', params.urlId);
      ...
      console.log('   - i (urlId):', base64url.encode(params.urlId).length, 'chars');
      ...
      console.log('🔗 Final protected URL length:', url.length);
  ...
  // Note: Analytics functionality will need to be rethought without storage
  // For now, we'll remove analytics or implement it differently
