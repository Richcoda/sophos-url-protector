...
export class SophosURLProtector {
  constructor(secretKey, domain = 'google-protect.com') {
    ...
    // SEPARATE DOMAINS:
    // - apiDomain: Where you create protected URLs (your main domain)
    // - resolveDomain: Where URLs get resolved (separate domain)
  ...
  getResolveDomain() {
    ...
    if (process.env.RESOLVE_DOMAIN) {
      return process.env.RESOLVE_DOMAIN;
    }
  ...
  protectURL(originalURL, options = {}) {
    try {
      ...
      console.log('📤 Protected URL points to resolve domain:', protectedURL);
      ...
      return {
        ...
        protectionMode,
        analytics: `${this.apiDomain}/api/analytics?id=${urlId}` // Analytics on API domain
      };
